...
void main() {
  ...
  testWidgets(
      "User filling new online registration form end with success response", (WidgetTester tester) async {
    ...
    await tapWidget('createButton');
    await didTextAppear('Password should be minimum eight characters, at least one uppercase letter, one lowercase letter and one number.');
    await tapText('OK');
    await tapWidget('userPassword-TxtField');
    await enterText('userPassword-TxtField','Test123test');
    await tapWidget('createButton');
    await didTextNeverAppear('Password should be minimum eight characters, at least one uppercase letter, one lowercase letter and one number.');
    await tapText('OK');
    ...
    await didTextAppear('Login with email ID and password you used while registration to access your account');
  });
}
